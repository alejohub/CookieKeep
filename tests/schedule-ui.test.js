...
for(const page of ['popup','options'])test(`${page}: visible mode persists and reflects shared configuration changes immediately`,async()=>{
 ...
 await import(`../src/${page}/${page==='popup'?'popup':'options'}.js?schedule-test=${page}`);await tick();assert.equal(get('version').textContent,'v1.1.5');get('interval').value='lastWindowClosed';
