...
for(const page of ['popup','options'])test(`${page}: visible mode persists and reflects shared configuration changes immediately`,async()=>{
 ...
 if(page==='popup')await get('interval').onchange();else{await get('settings').onclick();await all(get('dialog-actions')).find(e=>e.textContent==='Confirmar activación').onclick();}
 await tick();assert.deepEqual(state.cleanupSchedule,{mode:'lastWindowClosed'});assert.equal(get('interval').value,'lastWindowClosed');assert.equal(get('next').textContent,'Se limpiará al cerrar todas las ventanas.');assert.ok(calls.some(c=>c.type==='settings'&&c.token==='qa'));
 ...
 await send({type:'settings',schedule:{mode:'lastWindowClosed'},token:'qa'});await tick();assert.equal(get('interval').value,'lastWindowClosed');assert.equal(get('next').textContent,'Se limpiará al cerrar todas las ventanas.');
});
