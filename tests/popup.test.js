...
assert.match(markup,/<span class="muted popup-tagline">Protege lo importante\.<br>Limpia el resto\.<\/span>/);
...
test('popup reads version from the runtime manifest without changing snapshot or dashboard actions',async()=>{
  for(const version of ['9.8.7','2.3.4.5','0.0.0']){
    ...
    globalThis.chrome={runtime:{
      ...
      sendMessage:async()=>({ok:true,data:{host:version==='0.0.0'?null:'example.test',siteCount:12,siteBytes:2000,state:{whitelist:[]},nextRun:null,running:false}})
    }};
    ...
    assert.equal(elements.get('host').textContent,version==='0.0.0'?'Página no compatible':'example.test');
    assert.equal(elements.get('status').hidden,version==='0.0.0');assert.ok(!elements.get('status').textContent.includes('HTTP'));
    assert.equal(elements.get('protect').disabled,version==='0.0.0');
    assert.equal(elements.get('delete').disabled,version==='0.0.0');
