...
Version **1.1.10** adds exact preview candidate details to cleanup dialogs and consolidates readable typography. Recent cleanup, independent dashboard cards, dynamic version metadata and explicit GitHub links remain available. Earlier cleanup-policy, progress, cancellation, optional history and CSP safeguards remain in place. Source and Chromium ZIPs are tracked together. The published 1.1.1 archive is retained unchanged; the current archive is `releases/CookieKeep-v1.1.10-chromium.zip`.
...
Download and extract `releases/CookieKeep-v1.1.10-chromium.zip`. The manifest is at the archive root.
...
Shared typography uses the established 28px title, 18px section heading, 14px body and 13px secondary scale. Only the primary dashboard row data—domain, cookie count, approximate size and visits—is explicitly 15px; table headers, status badges and action buttons keep their prior sizing. The 1.1.10 release includes these UX improvements; previous release archives remain unchanged.
