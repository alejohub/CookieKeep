...
for(const path of files.filter(p=>p.endsWith('.html'))){const source=await readFile(path,'utf8');for(const match of source.matchAll(/<([a-z]+)\b[^>]*\b(src|href)="([^"#]+)"/g)){if(match[1]==='a' && match[2]==='href' && match[3].startsWith('https://'))continue;await stat(resolve(dirname(path),match[3]));}if(/<script(?![^>]*src=)/.test(source))throw new Error('Script inline incompatible con CSP');}
